...
CN=qwert,OU=admin,DC=xfusion,DC=com
...
PS C:\> $result = Set-iBMCLDAP -Session $Session -LDAPID 1 -LDAPAddress "ldap.xfusion.com" -LDAPPort 635 `
          -UserDomain 'CN=test,,DC=xfusion,DC=com' -BindDN test -BindPassword $BindPwd `
          -CertificateVerificationEnabled $true -CertificateVerificationLevel Allow `
          -GroupID 0 -GroupName qwert -GroupDomain 'CN=qwert,OU=admin,DC=xfusion,DC=com' `
