...
#>
  ...
  process {
    ...
    $ScriptBlock = {
      ...
      $Path = "/redfish/v1/Managers/$($RedfishSession.Id)/Actions/Oem/$OEM/Manager.ExportConfiguration"
...
#>
  ...
  process {
    ...
    $ScriptBlock = {
      ...
      $Logger.info($(Trace-Session $RedfishSession "Sending payload: $($Clone | ConvertTo-Json)"))
      $Path = "/redfish/v1/Managers/$($RedfishSession.Id)/Actions/Oem/$OEM/Manager.ImportConfiguration"
      $Response = Invoke-RedfishRequest $RedfishSession $Path 'Post' $payload
      return $Response | ConvertFrom-WebResponse
    }
