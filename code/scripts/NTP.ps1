...
Host                        : 192.168.1.1
ServiceEnabled              : True
PreferredNtpServer          : pre.xfusion.com
AlternateNtpServer          : alt.xfusion.com
...
PS C:\> Set-iBMCNTPSetting $session -ServiceEnabled $true
          -PreferredNtpServer 'pre.xfusion.com' -AlternateNtpServer 'alt.xfusion.com' `
    ...
    $ScriptBlock = {
      ...
      if ($ExtendInfo.'@odata.id' -like '/redfish/v1/TaskService/Tasks/*') {
        ...
        if ($ExtendInfo.Severity -ne $BMC.Severity.OK.ToString()) {
          ...
          $FakeTask.Oem.$OEM.TaskPercentage = $null
